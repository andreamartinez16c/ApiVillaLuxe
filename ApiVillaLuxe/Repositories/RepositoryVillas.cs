...
namespace ApiVillaLuxe.Repositories
{
    public class RepositoryVillas
    {
        ...
        public async Task<List<Villa>> GetVillasByProvinciaAsync(string provincia)
        {
            var villasAll = await GetVillasAsync();

            var villasFiltradas = villasAll.Where(v => v.NombreProvincia == provincia).ToList();

            return villasFiltradas;
        }
        ...
        public async Task<List<VillaTabla>> GetVillasByProvinciaAsync(int idProvincia)
        {
            return await this.context.VillasT
                .Where(v => v.IdProvincia == idProvincia)
                .ToListAsync();
        }
